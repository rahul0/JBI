  ...
  while((ent=readdir(dir))!=NULL){
    if(strchr(ent->d_name,'.')==NULL){
      ...
      */
      fp= fopen(filename,"r");
      assert(fp!=NULL);
      memset(filename,'\0',sizeof(char)*2000);
...
//        cout << "FACTOR[" <<i+1 << "," << j << "," << w <<"]=" << log(res) << "\n" ;
  ...
  void print_path(int i , int j , int gamma){
    ...
    if(i==0 ){
          if(gamma==0)
          //  cout << "gamma =0 in path tracing" << endl  ;
      return ;
    }
    else{
      bool isTopic_in_U=j==topic_count-1  ? true : false;
  ...
  void print_filled_table(document* doc){
    ...
    for(int i=0;i<word_index;i++)
      for(int j=0;j<topic_count;j++){
        for (int gamma=0;gamma<GAMMA ;gamma ++){
        //  double val=get_d_3D_m(F,i,j,gamma,topic_count,GAMMA);
//          if(val!=0  && j==topic_count-1)
  ...
  void  reconstruct_doc(document *doc){
    ...
    for(int i=0;i<N_d;i++){
      ...
      bool isWordUnderLined_Prediction=(path[i]==topic_count-1) ? true: false;
    ...
    myfile << html_end_tag ;
    myfile.close();
    //cout <<  doc->document_name << " = " << (double)match/(doc->mygamma)*100  << "  GAMMA= " << doc->mygamma << endl ;
...
/*
    ...
    cout <<  doc->document_name << " = " << accuracy <<   "  precision: " << precision  << " recall: "  << recall<< " F: "<< F  << endl ;
    */
   original_name.clear();
   file_name.clear();
   html_end_tag.clear();
...
string make_uppercase_string(string data){
  ...
  void run_test(document* d_curr){
    if(d_curr->mygamma==0){
  //  cout  << d_curr->document_name  << " SKIPPING    " << endl ;
      document_fee_mem(d_curr);
      return ;
    }
    //GAMMA=(d_curr->mygamma)+1 ;
    GAMMA=(d_curr->mygamma)+1 ;
    if(strcmp(d_curr->document_name,"p_373")==0){
    return ;
    }
   ...
   // for(int j=0;j<topic_count ;j++){
      ...
      // for(int gamma=GAMMA;gamma>=0;gamma--){
       for(int gamma=GAMMA-1;gamma>=0;gamma--){
            for(int j=0;j<topic_count ;j++){
  ...
  //  print_filled_table(d_curr) ;
int   max_state_topic=-1;
double max_val=-DBL_MAX;
int x=0;
for( x=0 ; x<topic_count;x++){
  double val=get_d_3D_m(F,N_d-1,x,GAMMA-1,topic_count,GAMMA);
 // cout << "state: " << x << " " << "val=" << val <<  endl ;
  if(val> max_val){
    max_val=val;
    max_state_topic=x;
  } 
...
//BUG fix here 
    ...
    path[N_d-1]=x;
    if(x==topic_count-1){
    print_path(N_d-1,x,GAMMA-2);
    }else{
    print_path(N_d-1,x,GAMMA-1);
    }
   // print_path(N_d-1,topic_count-1,GAMMA);
  ...
  int main(){
    //  print_system_config();
alpha_upper_N=462358;
alpha_upper_Y=53569.9;
alpha_comma_N=8.46637e-08;
alpha_comma_Y=2.51654e-07;
alpha_period_Y=0.38122;
alpha_period_N=1.0355;
