...
void init_variables(){
  ...
  assert(freq_single_den!=NULL);
    ...
    d_ptr=(document*)malloc(sizeof(document)*document_count);
    assert(d_ptr!=NULL);
    memset(d_ptr,0,sizeof(document)*document_count);
    ...
    last_maxima_theta_iw=(double*) malloc(sizeof(double)*topic_count*total_words_count);
    assert(last_maxima_theta_iw!=NULL);

    last_maxima_m_ij=(double*) malloc(sizeof(double)*topic_count*topic_count);
...
void init_model_parameters(){
   ...
   */ 
    //Normal distribution
    for(int i=0;i<topic_count;i++){
    double temp=phi_mu+gsl_ran_gaussian(rand_obj,sqrt(phi_sigma_square));
    PHI[i]=temp;
  }
...
void init_document_params(){
  ...
  while((ent=readdir(dir))!=NULL){
    if(strchr(ent->d_name,'.')==NULL){
     ...
     // d_ptr[updated_doc++] =current_doc_ptr ;
     // document * current_doc_ptr=d_ptr+ (updated_doc++) ;
      //Extract name
...
void make_int_words(document* doc,vector<string>& word_list ){
  ...
  const  int sz=doc->real_word_count;
  ...
  free(doc->word_caps);
  free(doc->comma_present_before);
  free(doc->period_present_before);
  memset(doc->priority_flag,false,sz);
  doc->hasPriorityWord=false;
...
void  update_m_ij(const int row,const int col,double * sum_den,uint32_t* freq_den,uint32_t*  freq_num){
    ...
    //M[row][col]=sampled_number;
    //  printf("M[%d][%d]=%f ",row,col,sampled_number);
}
...
void print_system_config(){
    ...
    printf("Linear normal model diagonal element only for  diag element is very high\n");
